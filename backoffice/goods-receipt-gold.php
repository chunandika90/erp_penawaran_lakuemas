 ...
 * di-generate otomatis (next_doc_number, docType 'plu') pas disimpan.
 */
    ...
    if (($_POST['action'] ?? '') === 'save_receipt') {
        ...
        try {
            ...
            foreach ($rows as $r) {
                ...
                $pluCode = next_doc_number($org['organization_id'], 'PLU');
                $insItem->execute([$org['organization_id'], $r['product_id'], $locationId, $r['stock_type_id'], $r['certificate_code'], $pluCode, $r['weight'], $projectId, $grId]);
            }
            $pdo->commit();
...
<div class="card txn-form-page" style="margin-bottom:20px;">
  ...
  <form method="post" id="gr-form">
    ...
    <table class="data-table" id="gr-lines-table" style="margin-bottom:10px;">
      <thead><tr><th style="width:26%;">Produk</th><th>Kode Sertifikat</th><th style="width:110px;">Berat (gr)</th><th style="width:180px;">Tipe Stock</th><th></th></tr></thead>
...
<div class="card">
  ...
  <table class="data-table">
    <thead><tr><th>No. Dokumen</th><th>Lokasi</th><th>Vendor</th><th class="num">Jumlah Barang</th><th>Tanggal</th></tr></thead>
    <tbody>
      ...
      <?php endforeach; ?>
      <?php if (!$recent): ?><tr><td colspan="5" style="text-align:center; color:var(--ink-muted);">Belum ada penerimaan.</td></tr><?php endif; ?>
    </tbody>
  </table>
</div>
